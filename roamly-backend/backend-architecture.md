# Roamly Backend Architecture

## Tech Stack
- **Framework:** Python FastAPI
- **Database:** SQLite (`roamly.db` local file).
- **Authentication:** JWT (JSON Web Tokens) for stateless user sessions.
- **Geocoding:** Static mapping for MVP (selected zip codes).
- **Payments:** Stripe API (Stubs for MVP).

## Data Model (SQLite)

### `users`
- `id`: TEXT (UUID)
- `email`: TEXT (Unique)
- `password_hash`: TEXT
- `is_premium`: INTEGER (0 or 1)
- `created_at`: TIMESTAMP

### `places` (POIs)
- `id`: TEXT (Primary Key)
- `name`: TEXT
- `category`: TEXT (food, attraction, outdoors, shopping)
- `rating`: REAL (0.0 to 5.0)
- `price_level`: INTEGER (1 to 4)
- `lat`: REAL
- `lng`: REAL
- `address`: TEXT
- `typical_duration`: INTEGER (Typical visit duration in minutes)

## API Endpoints

### Auth
- `POST /api/auth/register`: Register a new user.
- `POST /api/auth/login`: Authenticate and receive a JWT.

### Search & Itineraries
- `GET /api/itinerary`: 
  - Params: `zip_code`, `days`
  - Output: Ranked day-by-day itinerary.
- `GET /api/places/{id}`: Get detailed info for a specific place.

### Subscriptions
- `POST /api/subscribe/checkout`: Create a Stripe session placeholder.

## Ranking Algorithm
The core value of Roamly is the ranking of places. The score is a weighted blend:
1. **Review Score (40%)**: Normalized `rating / 5.0`.
2. **Cost Reasonableness (20%)**: Inverse price level. Lower price = higher score for budget travelers.
3. **Duration Fit (20%)**: Places that fit well into a ~90 minute window.
4. **Distance (20%)**: Proximity to the center of the provided zip code.

`Final Score = (RatingScore * 0.4) + (PriceScore * 0.2) + (DurationScore * 0.2) + (DistanceScore * 0.2)`

## Seed Data
We maintain an internal endpoint `/api/seed` that populates `roamly.db` with 20+ diverse places in major areas (e.g., NYC, LA, Chicago, Miami).
